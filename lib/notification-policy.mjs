...
export function getCurrentOjtWeek(startDate, totalWeeks = 18, now = new Date()) {
  const start = dateOnlyUtc(startDate)
  const current = dateOnlyUtc(now)
  const safeTotalWeeks = Math.max(1, Math.floor(Number(totalWeeks) || 18))
...
export function buildCurrentRoadmapChecklistNotification({
  ...
  const currentWeekNumber = getCurrentOjtWeek(
    roadmap.startDate || fallbackStartDate,
    roadmap.weeks.length || fallbackTotalWeeks,
    now
  )
  const week =
  ...
  return {
    ...
    message: `Week ${currentWeekNumber} - ${incompleteCount} checklist ${
