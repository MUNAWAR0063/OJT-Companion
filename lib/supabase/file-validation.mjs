...
export const PROFILE_AVATAR_MIME_TYPES = PHOTO_MIME_TYPES
...
export function validateModuleFile({ module, fileName, mimeType, sizeBytes }) {
  ...
  if (module === PROFILE_MODULE) {
    if (sizeBytes > PROFILE_AVATAR_MAX_BYTES) return { ok: false, message: "Profile picture must be 5 MB or smaller." }
    if (!PROFILE_AVATAR_MIME_TYPES.has(mimeType)) return { ok: false, message: "Profile picture must be JPG, PNG, or WebP." }
    return { ok: true }
  }
...
export function profileAvatarPath({ userId, timestamp, fileName }) {
  const extension = fileExtension(safeFileName(fileName)) || "jpg"
  return `${userId}/${PROFILE_MODULE}/avatar-${timestamp}.${extension}`
}
